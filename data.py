...
WIN_LINE_COLOR = (0, 255, 0)  # Kazanan çizgisi rengi (yeşil)
WIN_LINE_WIDTH = 3  # Kazanan çizgisi kalınlığı
WIN_LINE_ANIMATION_SPEED = 10  # Kazanan çizgisi animasyon hızı (daha büyük bir değer daha hızlı animasyon demektir)
...
# İkon yüklenir
icon = pygame.image.load(methods.icon_path_Settings)
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if methods.fullscreen_checkbox.collidepoint(mouse_pos):
                full_screen.toggle_fullscreen(screen)
                methods.WIDTH = screen.get_width()
                methods.HEIGHT = screen.get_height()
                methods.CELL_SIZE = methods.WIDTH // methods.BOARD_SIZE
                methods.fullscreen_checkbox = pygame.Rect(35, 100, 30, 30)
                methods.first_player_x_button = pygame.Rect(20, 150, 30, 30)
                methods.first_player_o_button = pygame.Rect(50, 150, 30, 30)
                methods.second_player_x_button = pygame.Rect(20, 200, 30, 30)
                methods.second_player_o_button = pygame.Rect(50, 200, 30, 30)
                methods.start_game_button = pygame.Rect(methods.WIDTH // 2 - 50, methods.HEIGHT - 100, 100, 50)
...
def main():
    # İkon yüklenir
    icon = pygame.image.load(methods.icon_path_game)
    # İkon ayarlanır
    pygame.display.set_icon(icon)
    global kazanan_x, kazanan_o, sayac, start_x, start_y, end_x, end_y
    pygame.init()
    ...
    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if game_over:
                    ...
                    if board[row][col] == '':
                        board[row][col] = turn
                        animate.animate_mark(screen, row, col, turn)
        ...
        if game_over:
            animate.draw_win_line(screen, (start_x, start_y), (end_x, end_y))
